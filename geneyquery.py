...
#            data = json.load(f)
    ...
    def filter_data(self):
        ...
        for sample in matching_samples:
            ...
            entire_row = self.geney_file_collection.tsv_file.read(self.geney_file_collection.tsv_map[sample][1]).rstrip(b"\n").split(b"\t")
    ...
    def __perform_discrete_filter(self, column_name, values_list):
        ...
        matching_samples = [item for item in range(0, len(desired_feature_data)) if (desired_feature_data[item].decode("UTF-8") in values_list)]
